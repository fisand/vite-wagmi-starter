...
export function presetShadcn(options: PresetShadcnOptions = {}): Preset<Theme> {
  return {
    ...
    rules: [
      ...
      [
        /^slide-in-from-top-?(\d+)?$/,
        ([, d]) => ({ '--un-enter-translate-y': typeof d === 'undefined' ? '-100%' : `-${Number(d) / 4}rem` }),
      ],
      [
      ...
      [
        ...
        ([, d]) => ({ '--un-enter-translate-x': typeof d === 'undefined' ? '-100%' : `-${Number(d) / 4}rem` }),
      ],
      [
      ...
      [
        /^slide-out-from-top-?(\d+)?$/,
        ([, d]) => ({ '--un-exit-translate-y': typeof d === 'undefined' ? '-100%' : `-${Number(d) / 4}rem` }),
      ],
      [
        /^slide-out-from-bottom-?(\d+)?$/,
        ([, d]) => ({ '--un-exit-translate-y': typeof d === 'undefined' ? '100%' : `${Number(d) / 4}rem` }),
      ],
      [
        /^slide-out-from-left-?(\d+)?$/,
        ([, d]) => ({ '--un-exit-translate-x': typeof d === 'undefined' ? '-100%' : `-${Number(d) / 4}rem` }),
      ],
      [
        /^slide-out-from-right-?(\d+)?$/,
        ([, d]) => ({ '--un-exit-translate-x': typeof d === 'undefined' ? '100%' : `${Number(d) / 4}rem` }),
      ],
    ],
  }
